...
class SyougiTest2 extends Frame implements Runnable, MouseListener {
	...
	public static void main(String args[]) throws ClassNotFoundException,
		...
		SyougiTest2 syougi = new SyougiTest2();
	...
	public void run() { // TODO Auto-generated
		...
		List<Koma> motigoma;
		while (true) {
			NormalPlay.PointInfo komaPointInfo;
			// cpu play
			try {
				// ランダムに手を決める
				komaPointInfo = NormalPlay.randomThink(field, cpuMotigomaLst);
				NormalPlay.normalPlay2(field, cpuMotigomaLst, "Cpu",
						komaPointInfo.nowPoint, komaPointInfo.newPoint);
				this.repaint();
			...
			if (playType.equals("normal")) {
				// 盤の中の駒を選択してプレイ
				try {
					NormalPlay.normalPlay(field, humanMotigomaLst, "Human");
				} catch (ClassNotFoundException | InstantiationException
						| IllegalAccessException | NoSuchMethodException
						| SecurityException | IllegalArgumentException
				...
				// 持ち駒の中の駒を選択してプレイ
				MotigomaPlay.motigomaPlay(field, humanMotigomaLst, "Human");
			}
			// paint実行
			this.repaint();
			try {
	...
	public void mouseClicked(MouseEvent e) {
		...
		// field
		if (point.x >= 330 && point.y >= (50 + 32) && point.x <= 900
				&& point.y <= 690) {
			System.out.println("click point in the field");
			System.out.println("x" + (point.x - 330) / 60);
			System.out.println("y" + (point.y - (50 + 32)) / 64);

		} else if (point.x >= (300 + 600) && point.y >= ((6 - 1) * 64 + 82)
				&& point.x <= (300 + 600)+46*5 && point.y <= ((6 - 1) * 64 + 82)+64*4) {
			System.out.println("click point in the motigoma");
			System.out.println("x" + (point.x - (300 + 600)) / 46);
			System.out.println("y" + (point.y - ((6 - 1) * 64 + 82)) / 64);
			System.out.println(((point.y - ((6 - 1) * 64 + 82))/64)*5 +((point.x - (300 + 600)) / 46)%5);
